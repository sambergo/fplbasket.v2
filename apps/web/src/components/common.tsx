...
export function ErrorPanel({ error, retry }: { error: Error; retry?: () => void }) { return <Card className="error-panel flex flex-col items-center gap-4 p-10 text-center"><span className="error-panel__icon grid size-12 place-items-center rounded-2xl"><AlertTriangle /></span><div><h2 className="text-lg font-bold">Couldn’t load this view</h2><p className="mt-1 text-sm">{error.message}</p></div>{retry && <Button variant="outline" onClick={retry}>Try again</Button>}</Card>; }
