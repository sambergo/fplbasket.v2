...
export function Standings() {
  ...
  const columns: ColumnDef<Row>[] = [
    ...
    { accessorFn: (row) => managers.get(row.managerId)?.playerName, id: "manager", header: "Manager", enableSorting: false, cell: ({ row }) => <div><div className="font-semibold">{managers.get(row.original.managerId)?.playerName}</div><div className="text-xs text-[#a6adc8]">{managers.get(row.original.managerId)?.teamName}</div></div> },
