...
export type DataTableColumnDef<T extends RowData> = ColumnDef<typeof features, T>;

export function DataTable<T extends RowData>({ data, columns, filterPlaceholder, onRowClick }: { data: T[]; columns: DataTableColumnDef<T>[]; filterPlaceholder?: string; onRowClick?: (row: T) => void }) {
  const [sorting, setSorting] = useState<SortingState>([]);
  const [filter, setFilter] = useState("");
  const table = useTable({ features, data, columns, state: { sorting, globalFilter: filter }, onSortingChange: setSorting, onGlobalFilterChange: setFilter });
  return <>
